<?php

/* Vila Wong
 * The home page for site
*/

#set page title and include header file:
define('TITLE', 'The Soup Place - Home');
include('includes/header.php');
?>

<!-- Main jumbotron for daily soup special -->
<div class="jumbotron home-jumbotron mb-0 p-0">
    <div class="mobile-overlay">
        <div class="container p-0">
            <div class="col-md-6">
                <a id="content" class="sr-only" title="Main content starts here"></a>
                <h1>Daily Special | Gazpacho</h1>
                <span class="sr-only">(Ingredients in daily special)</span>
                <p class="text-capitalize font-weight-bold">Tomatoes, cucumber, red bell pepper, onion, jalapeño, garlic, spices</p>
                <p><a class="btn btn-primary btn-main btn-lg" href="menu.php" role="button" title="Go to menu page">See Entire Menu &raquo;</a></p>
            </div>
        </div>
    </div>
</div>

<div class="bg-white">
    <div class="container ">
        <div class="row">
            <div id="procedure" class="col-md-6 pt-5 pb-4 py-md-5 ">
            <!-- PROCEDURE SECTION -->
            <label class="section-title">Ordering Procedure</label>
            <span class="section-subtitle">Follow procedure, get soup.</span>
                <ol>
                    <li>As you walk into the shop move immediately to your right.</li>
                    <li>Keep the line moving</li>
                    <li>Hold out your money</li>
                    <li>Speak your soup order in a loud, clear voice</li>
                    <li>Step to the left and receive</li>
                </ol>
            </div>

            <div id="rules" class="col-md-6 pt-4 pb-5 py-md-5 border-top">   
            <!-- RULES SECTION -->
            <label class="section-title">Rules</label>
            <span class="section-subtitle">Don't follow rules, then NO SOUP FOR YOU!</span>
                <ul>
                    <li>No embellishing on your order.</li>
                    <li>No extraneous comments.</li>
                    <li>No questions.</li>
                    <li>No compliments. </li>
                    <li>No cutting in line.</li>
                    <li>No kissing in line.</li>
                </ul>
            </div>
        </div>
    </div>
</div>

<?php
    include('includes/footer.php');
?>